...
|-----|---------------------|-----------------------------------|
| ~~`CN_INSTRUMENT.md`~~ **NOT ARCHIVED — see the note below.** | | |
...
> ⚠ **`CN_INSTRUMENT.md` was archived on 2026-07-17 and RESTORED the same day (user-caught).** Archiving it was
> a misjudgement: the **instruments** are dead, but the **machinery** is the starting point for the single
> highest-value open item in the programme — *restoring an exogenous existence validation* (board §A/§7). Its
> §2 (the exclusion restriction and why the unit is everything), §5 (multi-IV + **Hansen-J over-identification**,
> and why summing multi-locus CN is wrong), §6 (the CN/expression asymmetry — "the crux"), §7 (the corrected
> architecture) and §9's built-vs-gaps ledger are what a revival builds ON. A dead result is archivable; a live
> **design** is not. Its survivors were still extracted to `METHODS §8a` (that is the estimator SPEC, which
> belongs there under the one-home rule) — the doc keeps the design and the identifiability argument.
