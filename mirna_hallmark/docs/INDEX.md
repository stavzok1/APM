...
| [ARM_QUESTION_TAXONOMY.md](ARM_QUESTION_TAXONOMY.md) | active | The arm rung and its two neighbours: what can be asked of an arm alone, of an arm relative to its seed-family mates (`arm-in-family`), and of the family as an entity. Backs `apm-arm-question`. |
...
| [CARD_RUNG_DOCTRINE.md](CARD_RUNG_DOCTRINE.md) | active | ✅ §2 now records the **card_glossary** (per-column MEANING, 702/702, card-scoped) — the gap this doc used to name. What UNIT a number lives on — a property of (CARD, COLUMN), never of the column name. The five cards and their keys, rung vs agg_of, the invariance each rung implies, the `--check` workflow, and the add-a-column checklist. Read before adding or cross-card-comparing any column. |
...
| [derived/DOSSIER.html](derived/DOSSIER.html) | **⭐ THE HANDOFF DOSSIER** (2026-08-18) — ONE self-contained offline page condensing the whole subproject: the doc-currency table, per-axis standing **through MH-247** (so it covers the MH-208…247 arcs `STATE_OF_PLAY` does not), the five-rung card atlas with a **searchable 702-column index** (rung · `agg_of` · measured fill · domain), the do-not-rebuild graveyard, and ranked open work. Card numbers are **measured**, not quoted — producer: `analyses/ops/card_scan.py`. ⚠ A **VIEW over the one-home docs**, dated at compile time: where it and `DISCOVERY_REGISTRY.md` disagree, the registry wins. |
