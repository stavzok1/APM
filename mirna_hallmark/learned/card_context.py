...
def annotate() -> None:
    ...
    # ⭐ FAMILY-rung coupling + within-cell credit share (MH-187, corrected MH-188).
    # `coupling_fam` puts the family rung beside the card's single-ARM `coupling_*`.
    # ⚠ `arm_credit_share` (was `identity_arm`) is shipped, but its MEANING is not what the old name
    # implied: a HIGH share means one arm carries the signal and the POOL is essentially that arm, i.e.
    # **POOLING IS SAFE** — the OPPOSITE of `arm_resolvable`. It is NOT "this arm is the real regulator".
    rp = _read(OUT / "rung_parity.tsv")
    if rp is not None and "coupling_fam" in rp.columns:
        _c = ["gene", "arm", "coupling_fam"] + (["arm_credit_share"] if "arm_credit_share" in rp else [])
        blocks.append(rp[_c]); keys.append(["gene", "arm"])
