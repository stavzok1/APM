...
def load_sample_ids_from_manifest(manifest_path: Path) -> List[str]:
...
def load_sample_ids(ctx: CohortRunContext) -> List[str]:
    ...
    """
    out = load_sample_ids_from_manifest(ctx.methylation_subset_manifest)
    if out:
        return out
...
def to_tcga_sample_vial(x: str) -> str:
    tid = normalize_tcga_id(str(x))
