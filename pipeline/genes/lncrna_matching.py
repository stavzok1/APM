...
def _aggregate_lncrnas_per_gene(
    ...
    # Prior pipeline runs may already have ``col_name`` on ``genes`` (see DATA_STRUCTURE_ATLAS §8b);
    # merging again would suffix columns (_x/_y) and break downstream access by ``col_name``.
    if col_name in genes_out.columns:
        genes_out = genes_out.drop(columns=[col_name])
...
def _aggregate_genes_per_lncrna(
    ...
    # Legacy mode may load ``lncRNAs_with_genes_1000000bp.csv``, which can already carry
    # ``genes_within_*`` / ``num_genes`` from a previous save; drop before merge so ``col_name``
    # is unambiguous (Atlas §8b).
    stale = [c for c in (col_name, "num_genes") if c in lncrnas_out.columns]
    if stale:
        lncrnas_out = lncrnas_out.drop(columns=stale)
