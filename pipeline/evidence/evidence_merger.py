...
def _build_gene_links_for_element(
...
def build_unified_gene_links(
    ...
    if merged_links is None or len(merged_links) == 0:
        return pd.DataFrame(columns=["ENCODE_id", "gene_links"])

    rows: List[Dict[str, Any]] = []
    for enc, df_element in merged_links.groupby("ENCODE_id", sort=False):
        rows.append(
            {
                "ENCODE_id": enc,
                "gene_links": _build_gene_links_for_element(
                    df_element,
                    screen_exp_biosamples,
                    screen_exp_assays,
                    screen_comp_biosamples,
...
def validate_gene_links(elem_focus: pd.DataFrame) -> Dict[str, Any]:
    ...
    for gl in elem_focus["gene_links"].to_numpy(copy=False):
